...
% Economic and financial predictors used to predict stock return are a mixture of stationary and non-stationary variables. while the dependent variable stock return is stationary,
...
% , containing either the cointegrated predictors or non-cointegrated predictors.
% \cite{chang2003index} derives the limiting distributions of the nonlinear least square estimators and presents them as functions of Brownian motions and Brownian local time. 
% \begin{equation}
%     y_t = G(x^{\prime}\beta) + p(x,\alpha) + u_t
% \label{jp_model}
% \end{equation}
% where $x_t$ is an m-dimensional integrated process of order one, $p(x,\alpha)$ is a linear function defined on $\mathbb{R}^{m}$ and $u_t$ is a stationary error. In this model, $x^{\prime}\beta$ is called single-index and $G(x^{\prime}\beta)$ is the so called link function. \cite{chang2003index} derives the limiting distributions of the nonlinear least square estimators and presents them as functions of Brownian motions and Brownian local time. 
...
% In addition, we consider both stationary and non-stationary variables in our partially nonlinear model.

% Our model allows for lagged dependent variables because key macroeconomic/financial variables, such as the growth rate of GDP, the rate of unemployment and interest rates are typically autocorrelated. Failing to account for this autocorrelation will lead to serially correlated residuals. 

% Failing to account for this auto-correlation will lead to serially correlated residuals. To improve, we include the lagged dependent variable $y_{t-1}$ in the linear part of our model (\ref{PL model}) (\textcolor{red}{literatures}). 
...
% Since the nonlinear function $f\left(x_{t-1}^{\prime }\theta _{0},\gamma _{0}\right)$ could not be  

% Even though we do not known the optimal values beforehand, we can linearize our partially nonlinear model using Taylor series expansion and estimate the linearized model to obtain better initial values. 
...
% Before applying Taylor series expansion, we need to calculate the single-index $u_{t-1} = x^{\prime}_{t-1}\theta$. Since $x_1$ and $x_2$ are cointegrated,
